...
impl NodeKeypairs {
    ...
    pub fn public_key(&self) -> PublicKey {
        if let Some(key) = self.public_keys.bls {
            PublicKey::BlsShare(key)
        } else {
            PublicKey::Ed25519(self.public_keys.ed25519)
        }
    }
    ...
    pub fn set_bls_keys(&mut self, secret_share: BlsSecretKeyShare, public_set: PublicKeySet) {
        ...
        self.public_keys.bls = Some(public);
...
impl PublicKeys {
    ...
    /// Returns the PublicKeys serialised and encoded in z-base-32.
...
impl<'de> Deserialize<'de> for PublicKeys {
    fn deserialize<D: Deserializer<'de>>(deserialiser: D) -> Result<Self, D::Error> {
        ...
        Ok(PublicKeys { name, ed25519, bls })
    }
}

impl Ord for PublicKeys {
    fn cmp(&self, other: &PublicKeys) -> Ordering {
        utils::serialise(&self).cmp(&utils::serialise(other))
    }
}

impl PartialOrd for PublicKeys {
    fn partial_cmp(&self, other: &PublicKeys) -> Option<Ordering> {
        Some(self.cmp(other))
    }
}
