...
impl Data {
    ...
    pub fn new_public(pk: PublicKey, actor: ActorType, name: XorName, tag: u64) -> Self {
        Self {
            pk,
    ...
    pub fn new_private(pk: PublicKey, actor: ActorType, name: XorName, tag: u64) -> Self {
        Self {
            pk,
    ...
    pub fn append(&mut self, entry: Entry) -> Result<DataWriteOp<Entry>> {
        ...
        match &mut self.data {
            SeqData::Public(data) => data.append(entry, self.pk),
            SeqData::Private(data) => data.append(entry, self.pk),
        }
    }

    /// Apply a data CRDT operation.
    pub fn apply_data_op(&mut self, op: DataWriteOp<Entry>) -> Result<()> {
        self.check_permission(Action::Append, Some(op.pk), Some(&op.ctx))?;
    ...
    pub fn set_public_policy(
        ...
        match &mut self.data {
            SeqData::Public(data) => data.set_policy(PublicPolicy { owner, permissions }, self.pk),
    ...
    pub fn set_private_policy(
        ...
        match &mut self.data {
            SeqData::Private(data) => {
                data.set_policy(PrivatePolicy { owner, permissions }, self.pk)
            }
    ...
    pub fn apply_public_policy_op(&mut self, op: PolicyWriteOp<PublicPolicy>) -> Result<()> {
        ...
        match op.ctx {
            Some((ref policy_id, _)) => {
                self.check_permission(Action::Admin, Some(op.pk), Some(policy_id))?
            }
            None => self.check_permission(Action::Admin, Some(op.pk), None)?,
        }
