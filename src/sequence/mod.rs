...
impl Data {
    ...
    /// signed with, whilst the 'authority' is a unique identifier to be used as the
    ...
    pub fn in_range(
        ...
        let entries = match items {
            None => None,
            Some(entries) => Some(entries.iter().map(|(_, entry)| entry.to_vec()).collect()),
        };
    ...
    pub fn get(&self, index: Index, requester: Option<PublicKey>) -> Result<Option<&Vec<u8>>> {
        ...
        Ok(match &self.data {
            SeqData::Public(data) => data.get(index).map(|(_, entry)| entry),
            SeqData::Private(data) => data.get(index).map(|(_, entry)| entry),
        })
    }
    ...
    pub fn last_entry(&self, requester: Option<PublicKey>) -> Result<Option<&Entry>> {
        ...
        Ok(match &self.data {
            SeqData::Public(data) => data.last_entry().map(|(_, entry)| entry),
            SeqData::Private(data) => data.last_entry().map(|(_, entry)| entry),
        })
    }
    ...
    pub fn create_unsigned_append_op(&self, entry: Entry) -> Result<DataOp> {
        ...
        match &self.data {
            SeqData::Public(data) => data.create_append_op(None, entry, self.authority),
            SeqData::Private(data) => data.create_append_op(None, entry, self.authority),
        }
    }
...
mod tests {
    use crate::{
        utils, Error, Keypair, Result, Sequence, SequenceAddress, SequenceIndex, SequenceKind,
        SequenceOp, SequencePermissions, SequencePrivatePermissions, SequencePrivatePolicy,
    ...
    fn sequence_private_append_fails_when_no_perms_for_authority() -> anyhow::Result<()> {
        ...
        // let's append to both sequences with one first item
        let item1 = b"item1";
        let item2 = b"item2";
        let append_op1 = sign_sequence_op(
            replica1.create_unsigned_append_op(item1.to_vec())?,
        ...
        // lets check authority1 can read from replica1 and replica2
    ...
    fn generate_replicas(
        ...
        (1..max_quantity + 1).prop_map(move |quantity| {
            ...
            for _ in 0..quantity {
                let authority = Keypair::new_ed25519(&mut OsRng).public_key().to_string();

                let replica = Sequence::new_public(owner, authority, xorname, tag);
    ...
    proptest! {
        ...
        fn proptest_seq_doesnt_crash_with_random_data(
            ...
            let authority_keypair = Keypair::new_ed25519(&mut OsRng);
            let op_source_pk = authority_keypair.public_key();
            let sequence_name = XorName::random();

            let sequence_tag = 43_001u64;

            // Instantiate the same Sequence on two replicas
            let mut replica1 = Sequence::new_public(op_source_pk, "some_identifying_str".to_string(), sequence_name, sequence_tag);
            let mut replica2 = Sequence::new_public(op_source_pk, "some_identifying_str".to_string(), sequence_name, sequence_tag);

            // Set Actor1 as the owner
            let perms = BTreeMap::default();
            let owner_op = sign_sequence_public_policy_op( replica1.create_unsigned_public_policy_op(op_source_pk, perms)?, &authority_keypair)?;
            replica1.apply_public_policy_op(owner_op.clone())?;

            replica2.apply_public_policy_op(owner_op)?;

            // Append an item on replicas
            let append_op = sign_sequence_op(replica1.create_unsigned_append_op(s)?, &authority_keypair)?;
        ...
        fn proptest_seq_converge_with_many_random_data(
            ...
            for data in dataset {
                ...
                let append_op = sign_sequence_op( replica1.create_unsigned_append_op(data)?, &authority_keypair)?;
                replica1.apply_op(append_op.clone())?;
        ...
        fn proptest_dropped_data_can_be_reapplied_and_we_converge(
            ...
            // Instantiate the same Sequence on two replicas
            let mut replica1 = Sequence::new_public(op_source_pk, "some_identifying_str".to_string(), sequence_name, sequence_tag);
            let mut replica2 = Sequence::new_public(op_source_pk2, "some_identifying_str2".to_string(), sequence_name, sequence_tag);
            ...
            for (data, delivery_chance) in dataset {
                    let op = sign_sequence_op(replica1.create_unsigned_append_op(data)?, &authority_keypair)?;
        ...
        fn proptest_converge_with_shuffled_ops_including_bad_ops_which_error_and_are_not_applied(
            ...
            let mut bogus_replica = Sequence::new_public(owner_keypair.public_key(), "authority".to_string(), xorname, tag);
            let perms = BTreeMap::default();
