...
impl Data {
    ...
    pub fn apply_op(&mut self, op: DataOp) -> Result<()> {
    ...
    /*
        /// Returns the current public policy.
        pub fn policy<P>(&self) -> &P {
            self.data.policy()
            /*match &self.data {
                SeqData::Public(data) => data.policy(),
                SeqData::Private(data) => data.policy(),
            }*/
        }
    */
    ...
    pub fn check_permission(&self, action: Action, requester: Option<PublicKey>) -> Result<()> {
        let requester = requester.unwrap_or(self.operations_pk);
        match &self.data {
    ...
    /// Returns the PK which the messages are signed with for this replica.
    // TODO: review if we still need this PK in this layer
    pub fn replica_signer(&self) -> PublicKey {
        self.operations_pk
    }
}
...
mod tests {
    use crate::{
        utils, Error, Keypair, PublicKey, Result, Sequence, SequenceAddress, SequenceEntry,
        SequenceIndex, SequenceKind, SequenceOp, SequencePermissions, SequencePrivatePermissions,
    ...
    fn sign_sequence_op(mut op: SequenceOp, keypair: &Keypair) -> Result<SequenceOp> {
    ...
    fn create_public_seq_replicas(count: usize) -> (Keypair, Vec<Sequence>) {
        let source_keypair = Keypair::new_ed25519(&mut OsRng);
        let actor = source_keypair.public_key();
        let sequence_name = XorName::random();
        let sequence_tag = 43_000;
        let replicas: Vec<Sequence> = vec![0..count + 3]
            .iter()
            .map(|_| {
                Sequence::new_public(
                    actor,
                    "app-instance-1".to_string(),
                    sequence_name,
                    sequence_tag,
                    None,
                )
    ...
    fn sequence_append_entry_and_apply() -> Result<()> {
        let (source_keypair, mut replicas) = create_public_seq_replicas(2);

        let entry1 = b"value0".to_vec();
        let entry2 = b"value1".to_vec();

        let op1 = sign_sequence_op(
            replicas[0].create_unsigned_append_op(entry1.clone())?,
            &source_keypair,
        )?;
        replicas[0].apply_op(op1.clone())?;

        let op2 = sign_sequence_op(
            replicas[0].create_unsigned_append_op(entry2.clone())?,
            &source_keypair,
        )?;
        replicas[0].apply_op(op2.clone())?;

        // we apply the operations in different order to replica2,
        // just to verify that doesn't affect the result
        replicas[1].apply_op(op2)?;
        replicas[1].apply_op(op1)?;

        assert_eq!(replicas[0].len(None)?, 2);
        assert_eq!(replicas[1].len(None)?, 2);

        let index_0 = SequenceIndex::FromStart(0);
        let first_entry = replicas[0].get(index_0, None)?;
        assert_eq!(first_entry, Some(&entry1));
        assert_eq!(first_entry, replicas[1].get(index_0, None)?);

        let index_1 = SequenceIndex::FromStart(1);
        let second_entry = replicas[0].get(index_1, None)?;
        assert_eq!(second_entry, Some(&entry2));
        assert_eq!(second_entry, replicas[1].get(index_1, None)?);

        let last_entry = replicas[0].last_entry(None)?;
        assert_eq!(last_entry, Some(&entry2));
        assert_eq!(last_entry, replicas[1].last_entry(None)?);
    ...
    fn sequence_public_create_policy_op_and_append_succeeds_when_perms_updated_for_actor(
        ...
        let append_op = sign_sequence_op(
            replica2.create_unsigned_append_op(item2.to_vec())?,
            &op_source_pk2_keypair,
        )?;
        replica2.apply_op(append_op.clone())?;
        replica1.apply_op(append_op)?;
        assert_eq!(replica1.len(None)?, replica2.len(None)?);
    ...
    fn sequence_causality_between_data_and_policy_ops() -> anyhow::Result<()> {
        ...
        replica3.apply_op(append_op.clone())?;
        ...
        check_not_causally_ready_failure(replica2.apply_op(append_op.clone()))?;
        assert_eq!(replica2.len(None)?, 0);
    ...
    proptest! {
        ...
        fn proptest_seq_doesnt_crash_with_random_data(
            ...
            let append_op = sign_sequence_op(replica1.create_unsigned_append_op(s)?, &op_source_pk_keypair)?;
            replica1.apply_op(append_op.clone())?;
        ...
        fn proptest_seq_converge_with_many_random_data(
            ...
            for data in dataset {
                // Append an item on replica1
                let append_op = sign_sequence_op( replica1.create_unsigned_append_op(data)?, &op_source_pk_keypair)?;
                replica1.apply_op(append_op.clone())?;
        ...
        fn proptest_we_converge_with_ownership_changes(
            ...
            for (data, policy_change_chance) in dataset {
                let op = sign_sequence_op(replica1.create_unsigned_append_op(data)?, &op_source_pk_keypair)? ;
        ...
        fn proptest_converge_with_shuffled_ops_including_bad_ops_which_error_and_are_not_applied(
            ...
            for data in bogus_dataset {
                let bogus_op = sign_sequence_op( bogus_replica.create_unsigned_append_op(data)?, &owner_keypair)?;
