...
use crate::shared_types::{
    to_absolute_range, to_absolute_version, Address, ExpectedVersions, Owner, Scope, User, Value,
    ...
    C: AccessListTrait,
{
    ...
    pub fn owner_history_range(&self, start: Version, end: Version) -> Option<Vec<&Owner>> {
        ...
        Some(self.owners[range].iter().collect())
    }
...
impl Sequence {
    ...
    pub fn is_allowed(&self, access: AccessType, user: &PublicKey) -> bool {
        ...
        if let (Public(_), Read) = (self, access) {
            return true;
        }
        match (self, access) {
