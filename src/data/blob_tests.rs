...
fn deterministic_name() {
    ...
    let blob1 = PrivateBlob::new(data1.clone(), owner1);
    let blob2 = PrivateBlob::new(data1, owner2);
    let blob3 = PrivateBlob::new(data2.clone(), owner1);
    let blob3_clone = PrivateBlob::new(data2, owner1);

    assert_eq!(blob3, blob3_clone);

    assert_ne!(blob1.name(), blob2.name());
    assert_ne!(blob1.name(), blob3.name());
    assert_ne!(blob2.name(), blob3.name());
}
